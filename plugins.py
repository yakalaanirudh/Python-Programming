...
def mono(image, sepia=False):
    ...
    for row in image:
        for col in row:
            gray=int(0.3*col.red+0.6*col.green+0.1*col.blue)
            col.red=gray
            col.green=gray
            col.blue=gray
    if sepia==True:
        for row in image:
            for col in row:
                col.green=int(0.6*col.green)
                col.blue=int(0.4*col.blue)
    return True
    #return False
    ...
    # Flip the image vertical
        for i in range(0, int(rows/2)):
            for j in range(0, int(cols)):
                image[i][j], image[rows - i - 1][j] = image[rows - i - 1][j], image[i][j]
    else:
        for i in range(0, rows):
            for j in range(0, int(cols / 2)):
                image[i][j], image[i][cols - j - 1] = image[i][cols - j - 1], image[i][j]
    return True
    #return False
...
def transpose(image):
    ...
    image2 = [[0 for x in range(height)] for y in range(width)]

    for row in range(height):
        for col in range(width):
            image2[col][row] = image[row][col]
